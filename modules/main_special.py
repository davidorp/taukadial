...
def set_up(config, train_dataloader, device, fold = 0):
    ...
    model.load_state_dict(torch.load(f'logs/multimodal_model_regression/model_fold_{fold}.pth'))
    ...
    if config.train.cross_validation:
        ...
        for fold, (train_indices, test_indices) in enumerate(kf.split(dataset)):
            if fold == spected_fold:
                ...
                model, optimizer, lossfn, lr_scheduler = set_up(config, train_dataloader, device, best_fold)
